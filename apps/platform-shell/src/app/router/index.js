...
const LauncherPage = lazy(() => import('../../components/Platform/LauncherPage'));
  ...
  if (!hasAppAccess(appId)) {
    return (
      <div className="shell-card p-8">
        ...
        <h1 className="mt-3 text-2xl font-semibold text-slate-900">This app is not assigned to your account.</h1>
        ...
        <Link
          to="/apps"
          className="mt-6 inline-flex rounded-full border border-slate-200 bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white no-underline transition hover:bg-slate-800"
        >
          Return to launcher
        </Link>
      </div>
...
export const createAppRouter = () =>
  createBrowserRouter([
    { path: '/', element: <RootRedirect /> },
    {
      ...
      children: [
        { path: '/login', element: withSuspense(LoginForm, 'Preparing sign in...') },
        { path: '/register', element: withSuspense(RegisterForm, 'Preparing registration...') },
      ],
    },
    {
      element: <ProtectedRoute />,
      children: [
        { path: '/apps', element: withSuspense(LauncherPage, 'Loading launcher...') },
        {
