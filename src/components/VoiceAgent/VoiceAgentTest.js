...
const VoiceAgentTest = () => {
  ...
  const startCall = async () => {
    ...
    try {
      ...
      setError(startError.response?.data?.detail || startError.response?.data?.message || 'Failed to start call');
    } finally {
      setLoading(false);
    }
  };
  ...
  const endCall = async () => {
    ...
    try {
      ...
      setError(endError.response?.data?.detail || endError.response?.data?.message || 'Failed to end call');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-[0.78rem] uppercase tracking-[0.32em] text-white/68">Voice Testing</p>
          <h1 className="mt-3 text-3xl font-semibold tracking-[-0.03em] text-white">Voice Agent Testing</h1>
          <p className="mt-2 max-w-3xl text-sm leading-7 text-white/72">
            Test active voice agents for {selectedTenantData?.name || 'your selected tenant'} with browser-based
            simulation or live Twilio-powered calls.
          </p>
        </div>
        <button
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_420px]">
          <div className="space-y-6">
            <section className="rounded-[28px] border border-white/8 bg-white/[0.04] p-5">
              <div className="mb-5">
                <h2 className="text-xl font-semibold text-white">Voice Agent Controls</h2>
                <p className="mt-2 text-sm leading-7 text-white/62">
                  Select a tenant, choose an active agent, and decide whether to test in-browser or place a live call.
                </p>
              </div>

              <div className="grid gap-4">
                <div>
                  <label className="mb-2 block text-sm font-medium text-white/84">Tenant</label>
                  <select value={selectedTenant} onChange={(e) => setSelectedTenant(e.target.value)} className="shell-input">
                    {tenants.map((tenant) => (
                ...
                <div>
                  <label className="mb-2 block text-sm font-medium text-white/84">Select Agent to Test</label>
                  {agents.length > 0 ? (
                    <select value={selectedAgent} onChange={(e) => setSelectedAgent(e.target.value)} className="shell-input">
                      {agents.map((agent) => (
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/6 px-4 py-4 text-sm text-white/60">
                      ...
                      <Link to="/app/appointment-setter/voice-agents" className="text-sky-100 no-underline hover:text-white">
                ...
                {selectedAgentData && (
                  <div className="rounded-[22px] border border-sky-300/18 bg-sky-300/10 p-4">
                    <div className="flex items-start gap-3">
                      <User className="mt-0.5 h-5 w-5 text-sky-200" />
                      <div className="text-sm">
                        <p className="font-semibold text-sky-100">{selectedAgentData.name}</p>
                        <p className="mt-1 text-sky-100/82">
                          <span className="font-medium">Service:</span> {selectedAgentData.service_type}
                        </p>
                        <p className="text-sky-100/82">
                          <span className="font-medium">Language:</span> {selectedAgentData.language}
                        </p>
                        {agentPhone ? (
                          <p className="mt-1 flex items-center text-emerald-100">
                ...
                <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-4">
                  <label className="flex items-center">
                    ...
                    />
                    <span className="ml-3 text-sm text-white/82">Test Mode (LiveKit in browser)</span>
                  </label>
                </div>
                        ...
                        <div className="flex items-center gap-2">
                          {twilioIntegration ? (
                            <>
                              ...
                              <span className="text-sm font-medium text-emerald-100">Twilio Integration Active</span>
                            </>
                          ) : (
                            <>
                              <XCircle className="h-5 w-5 text-rose-200" />
                              <span className="text-sm font-medium text-rose-100">Twilio Integration Required</span>
                            </>
                          )}
                        </div>
                        {twilioIntegration && <span className="text-xs text-white/52">{twilioIntegration.phone_number}</span>}
                      </div>
                      {!twilioIntegration && (
                        <p className="mt-3 text-xs leading-6 text-white/58">
                          Please configure Twilio integration before placing live calls.
                        </p>
                      )}
                    </div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {!callActive ? (
                  <button
                    onClick={startCall}
                    disabled={loading}
                    className="inline-flex items-center gap-2 rounded-2xl bg-[#0f9f6e] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#0c8a5f] disabled:cursor-not-allowed disabled:opacity-50"
                  >
                  ...
                  <button
                    onClick={endCall}
                    disabled={loading}
                    className="inline-flex items-center gap-2 rounded-2xl bg-[#dc2626] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#c81e1e] disabled:cursor-not-allowed disabled:opacity-50"
                  >
              ...
              {callActive && (
                <div className="mt-6 rounded-[22px] border border-emerald-300/18 bg-emerald-300/10 p-4">
                  <h3 className="text-sm font-semibold text-emerald-100">Call Active</h3>
                  <div className="mt-3 grid gap-2 text-sm text-emerald-100/88">
                    ...
                    {selectedAgentData && <p>Agent: {selectedAgentData.name}</p>}
          ...
          <div className="space-y-6">
            <section className="rounded-[28px] border border-white/8 bg-white/[0.04] p-5">
              <div className="mb-4 flex items-end justify-between gap-3">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-7 text-white/62">Recent sessions for the selected tenant.</p>
                </div>
                <button
                  onClick={fetchCallHistory}
                  className="inline-flex items-center rounded-2xl border border-white/10 bg-white/6 px-3 py-2 text-sm text-white transition hover:bg-white/10"
                >
                  Refresh
                </button>
              </div>
              ...
              <div className="space-y-3">
                {callHistory && callHistory.length > 0 ? (
                  callHistory.map((call) => (
                    <div key={call.session_id || call.call_id || Math.random()} className="rounded-[20px] border border-white/8 bg-white/[0.03] p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-sm font-semibold text-white">{call.service_type || 'Unknown Service'}</p>
                          <p className="mt-1 text-xs text-white/54">
                            {call.started_at ? new Date(call.started_at).toLocaleString() : 'Unknown time'}
                          </p>
                          <p className="mt-1 text-xs text-white/54">Mode: {call.test_mode ? 'Test' : 'Live'}</p>
                        </div>
                        <span
            ...
            <section className="rounded-[28px] border border-sky-300/16 bg-sky-300/10 p-5">
              <h2 className="text-lg font-semibold text-sky-100">Integration Information</h2>
              <div className="mt-3 space-y-3 text-sm leading-7 text-sky-100/88">
                <p>
                  <strong>Test Mode (LiveKit):</strong> Use this mode to test your voice agent directly in the browser
                  without making actual phone calls.
                </p>
                <p>
                  <strong>Live Mode (Twilio):</strong> Use this mode to place real calls through your Twilio setup.
                </p>
                <p>
                  <strong>Features:</strong> Speech-to-text, text-to-speech, AI conversation flow, appointment booking,
